...
<div class='w-4/4 bg-pastel-50 h-4/4 m-10 mt-5 shadow rounded relative flow-root'>
    <div class='border border-2px h-9 bg-white text-center'>
      <label class='font-serif ml-2 text-2xl'>Resultados de alumno: {{auth()->user()->name}}</label>
    </div>
    ...
    @else
        ...
        <div class="flex justify-end">
            <a href="{{route('homealumno.descargarPDFAlumno')}}" class="btn btn-sm btn-primary"><button class="bg-blue-400 hover:bg-blue-500 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center"><svg class="fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z"/></svg><span>Descargar PDF</span></button></a>
        </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th class="border border-gray-400 px-4 py-2 text-gray-800">Docente</th> 
                    ...
                    <tbody>
                        @foreach ($calFinal as $calFinales)
                            ...
                            {{-- <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                ...
                                <td class="px-6 py-4">{{$calFinales->docente}}</td>
                    ...
                    <tbody>
                        @foreach ($listaCal as $calificaciones)
                            ...
                            {{-- <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                ...
                                <td class="px-6 py-4">{{$calificaciones->docente}}</td>
